...
export const ExerciseCard: React.FC<ExerciseCardProps> = ({ exercise, onSelect, compact = false }) => {
  ...
  return <button
    ...
    className="group relative text-left flex flex-col h-full focus:outline-none focus-visible:ring-2 focus-visible:ring-[#FF6B1A] rounded-2xl overflow-hidden bg-[#0D1116] border border-white/[0.055] hover:border-white/[0.13] transition-colors"
  ...
  >
    ...
    <div className={`relative ${compact ? 'h-28' : 'h-44'} overflow-hidden bg-[#090C10]`}>
      ...
      <div className="absolute inset-x-0 bottom-0 h-20 opacity-70 pointer-events-none" style={{ background: `linear-gradient(to top, ${visual.glow}, transparent)` }} />
    ...
    <div className="p-4 sm:p-5 flex flex-col flex-1">
      ...
      <div className="mt-auto pt-4">
        <div className="border-t border-white/[0.055] pt-3 flex items-end justify-between gap-3">
          ...
          <span className="text-xs font-bold flex items-center gap-1 shrink-0" style={{ color: visual.accent }}>Abrir <ArrowRight className="w-3.5 h-3.5 group-hover:translate-x-0.5 transition-transform" /></span>
        </div>
      </div>
    </div>
  </button>;
